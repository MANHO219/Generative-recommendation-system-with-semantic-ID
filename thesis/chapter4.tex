...
实验使用Yelp数据集的Philadelphia城市子集，该子集包含11,711个POI和675,543条用户评论记录。

\begin{table}[htbp]
\centering
\caption{Philadelphia数据集统计}
...
\textbf{补充说明}：原始Yelp数据集全量包含117,788个POI，覆盖24个州、1,261个城市。Philadelphia作为最大单城子集，POI数量为全量数据集的约9.9\%，具有充足的样本规模用于训练语义ID模型和LLM推荐模型。
...
\begin{itemize}
    \item \textbf{FPMC}~\cite{rendle2010factorifying}：因子分解个性化马尔可夫链（Factorized Personalized Markov Chains），RecSys 2010。该方法将矩阵分解与一阶马尔可夫链结合，建模用户偏好与序列转移的交互效应，是序列推荐领域经典基线。
    \item \textbf{STAN}~\cite{wang2021spatial}：时空注意力网络（Spatial-Temporal Attention Network），WWW 2021。该方法在自注意力机制基础上显式建模物品之间的空间距离与时间间隔关系，适合处理带有地理标签的POI推荐任务。
    \item \textbf{SASRec}~\cite{kang2018self}：自注意力序列推荐（Self-Attentive Sequential Recommendation），ICDM 2018。该方法利用Transformer架构中的多头自注意力机制从用户行为序列中自适应地学习长期依赖与短期偏好，是当前最具竞争力的非LLM序列推荐基线之一。
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{推荐效果对比}：与FPMC、STAN、SASRec等基线方法进行对比。
